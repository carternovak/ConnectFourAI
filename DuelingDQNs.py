...
class DuelingDQNs:
    ...
    def __init__(self, height = 6, width = 7, x = 4) -> None:
        ...
        self.dqn_one = DQNAgent(env_one, board_height=height, board_width=width, action_states=action_size, batch_size=128, epsilon=.999, epsilon_decay=0.01, min_epsilon=0.05, gamma=.8, lr=0.0001)
        board_two = ConnectXBoard(height=height, width=width, x=x)
        env_two = ConnectXGym(board_two,  None, 1, -1)
        self.dqn_two = DQNAgent(env_two, board_height=height, board_width=width, action_states=action_size, batch_size=128, epsilon=.999, epsilon_decay=0.01, min_epsilon=0.05, gamma=.8, lr=0.0001)
