...
class DuelingDQNs:
    ...
    def __init__(self, conv=False, height = 7, width = 6, x = 4) -> None:
        state_size = height * width
        action_size = width
        board_one = ConnectXBoard(height=height, width=width, x=x)
        env_one = ConnectXGym(board_one,  None, 1, -1)
        self.dqn_one = DQNAgent(env_one, conv_model=conv, board_height=height, board_width=width, action_states=action_size, batch_size=128, epsilon=.999, epsilon_decay=0.01, min_epsilon=0.05, gamma=.5, lr=0.0001)
        board_two = ConnectXBoard(height=height, width=width, x=x)
        env_two = ConnectXGym(board_two,  None, 1, -1)
        self.dqn_two = DQNAgent(env_two, conv_model=conv, board_height=height, board_width=width, action_states=action_size, batch_size=128, epsilon=.999, epsilon_decay=0.01, min_epsilon=0.05, gamma=.5, lr=0.0001)
